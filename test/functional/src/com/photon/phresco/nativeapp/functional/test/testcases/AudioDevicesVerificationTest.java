...
public class AudioDevicesVerificationTest extends TestCase {
	...
	public AudioDevicesVerificationTest(Solo soloTest) {
	 ...
	 *
	 */
	public void testAudioDevicesVerificationTest(Instrumentation instrumentation) throws TestException {
...
//            info.parser(instrumentation);
			...
			for (int i = 0; i < 20; i++) {
				Log.i(TAG, "**** checking for Audio Devices item*****");
				this.activityName = soloBrowse.getCurrentActivity().getClass().getSimpleName();
				Log.i(TAG, "The activity name------------>" + this.activityName);
				if (soloBrowse.waitForText("Audio Devices")) {
					Log.i(TAG, "********Enter into if() condition and clicking on Audio Devices******");
					// click on Audio Devices
					soloBrowse.clickInList(4);
					break;
				}
				Log.i(TAG, "*****waiting for Audio Devices item*****");
				soloBrowse.waitForText("Audio Devices", 1, 2000);
			}

			for (int i = 0; i < 20; i++) {
				this.activityName = soloBrowse.getCurrentActivity().getClass().getSimpleName();
				Log.i(TAG, "checking for ***Pearstone*** in ------------>" + this.activityName);
				if (soloBrowse.waitForText("Pearstone")) {
					Log.i(TAG, "****if() condition and clicking on ****Pearstone*****Item*****");
					soloBrowse.clickInList(1);
					break;
				} else {
					Log.i(TAG, "waiting for  *****Pearstone**** item in ------------>" + this.activityName);
					soloBrowse.waitForText("Pearstone", 1, 2000);
			...
			for (int i = 0; i < 30; i++) {
				...
				if (soloBrowse.waitForText("Pearstone", 1, 0, false, true)) {
					// if (soloBrowse.waitForText("Pearstone 11.6")) {
