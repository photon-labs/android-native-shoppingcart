 ...
 *
 */
public class CamerasVerificationTest extends TestCase {
	...
	private static final String TAG = "*******CamerasVerificationTest****";

	public CamerasVerificationTest(Solo soloTest) {
	 ...
	 *
	 */
	public void testCamerasVerificationTest(Instrumentation instrumentation) throws TestException {

		try {
			// click on Browse button
			// soloBrowse.clickOnImageButton(1);
			Log.i(TAG, "------It is testCamerasVerificationTest()-----------");
...
//            info.parser(instrumentation);
			...
			if (this.activityName.equalsIgnoreCase("MainActivity")) {
				...
				Log.i(TAG, "------ testCamerasVerificationTest failed-----------");
			...
			for (int i = 0; i < 20; i++) {
				Log.i(TAG, "**** checking for Cameras item*****");
				this.activityName = soloBrowse.getCurrentActivity().getClass().getSimpleName();
				Log.i(TAG, "The activity name------------>" + this.activityName);
				if (soloBrowse.waitForText("Cameras")) {
					Log.i(TAG, "********Enter into if() condition and clicking on Cameras******");
					// click on Cameras
					soloBrowse.clickInList(5);
					break;
				}
				Log.i(TAG, "*****waiting for Cameras item*****");
				soloBrowse.waitForText("Cameras", 1, 2000);
			}

			for (int i = 0; i < 20; i++) {
				this.activityName = soloBrowse.getCurrentActivity().getClass().getSimpleName();
				Log.i(TAG, "checking for ***Canon*** in ------------>" + this.activityName);
				if (soloBrowse.waitForText("Canon")) {
					Log.i(TAG, "****if() condition and clicking on ****Canon*****Item*****");
					soloBrowse.clickInList(1);
					break;
				} else {
					Log.i(TAG, "waiting for  *****Canon**** item in ------------>" + this.activityName);
					soloBrowse.waitForText("Canon", 1, 2000);
			...
			for (int i = 0; i < 30; i++) {
				...
				if (soloBrowse.waitForText("Canon", 1, 0, false, true)) {
					// if (soloBrowse.waitForText("Canon 11.6")) {
