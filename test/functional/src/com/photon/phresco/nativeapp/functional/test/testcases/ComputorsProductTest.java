 ...
 *
 */
public class ComputorsProductTest extends TestCase {
	...
	public ComputorsProductTest(Solo soloTest) {
	 ...
	 *
	 */
	public void testComputorsProducts(Instrumentation instrumentation) throws TestException {

		try {
			// click on Browse button
			// soloBrowse.clickOnImageButton(1);
			Log.i(TAG, "------It is testComputorsProducts()-----------");
...
//            info.parser(instrumentation);
			...
			for (int i = 0; i < 20; i++) {
				Log.i(TAG, "**** checking for Computers item*****");
				this.activityName = soloBrowse.getCurrentActivity().getClass().getSimpleName();
				Log.i(TAG, "The activity name------------>" + this.activityName);
				if (soloBrowse.waitForText("Computers")) {
					Log.i(TAG, "********Enter into if() condition and clicking on Computers******");
					// click on Computers
					soloBrowse.clickInList(2);
					break;
				}
				Log.i(TAG, "*****waiting for Computers item*****");
				soloBrowse.waitForText("Computers", 1, 2000);
			}

			for (int i = 0; i < 20; i++) {
				this.activityName = soloBrowse.getCurrentActivity().getClass().getSimpleName();
				Log.i(TAG, "checking for ***Apple*** in ------------>" + this.activityName);
				if (soloBrowse.waitForText("Apple")) {
					Log.i(TAG, "****if() condition and clicking on ****Apple*****Item*****");
					soloBrowse.clickInList(1);
					break;
				} else {
					Log.i(TAG, "waiting for  *****Apple**** item in ------------>" + this.activityName);
					soloBrowse.waitForText("Apple", 1, 2000);
			...
			for (int i = 0; i < 30; i++) {
				...
				if (soloBrowse.waitForText("Apple", 1, 0, false, true)) {
